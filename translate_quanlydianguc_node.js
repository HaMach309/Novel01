/**
 * Dịch chương 1-52 từ tiếng Trung sang tiếng Việt bằng package translate.
 * Chạy: node translate_quanlydianguc_node.js [--test]
...
async function translateChunk(translate, text) {
  if (!text.trim()) return '';
  const maxLen = 4500;
  if (text.length <= maxLen) {
    try {
      const r = await translate(text, { from: 'zh', to: 'vi' });
      return r || text;
    } catch (e) {
  ...
  while (pos < text.length) {
    ...
    try {
      const t = await translate(chunk, { from: 'zh', to: 'vi' });
      parts.push(t || chunk);
    } catch (e) {
      parts.push(chunk);
    }
...
async function main() {
  ...
  for (const ch of chapters) {
    console.log(`Chương ${ch.num}: ${ch.titleCn.substring(0, 30)}...`);

    const titleVn = await translateChunk(translate, ch.titleCn);
    await sleep(200);

    const bodyVn = await translateChunk(translate, ch.body);
